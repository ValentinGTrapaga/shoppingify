    ...
    <li className='relative z-10 list-none' onMouseEnter={() => { setMouseOn(true) }} onMouseLeave={() => { setMouseOn(false) }}>
      ...
      {mouseOn &&
        <>
          ...
          <span className='absolute bg-slate-600 text-xs top-1/2 left-8 -translate-y-1/2 px-2 py-1 text-white rounded-sm shadow-sm z-[9999] flex font-semibold'><p className='flex-1 text-center whitespace-nowrap'>{name}</p></span>
        </>
      }
    </li>)
}
